...
class JdbcRepository(ProcessRepository):
    ...
    async def lock_instance_for_update(self, instance_id: int) -> None:
        """§27 修复（2026-09-19）：悲观锁 process_instance 行

        PG/MySQL 用 `SELECT ... FOR UPDATE`（InnoDB/PG 行锁直到事务结束）。
        必须在 with_tx 事务内调用：调用方（facade）负责事务边界。
        同 instance.id 的 _create_task 串行化，防止多入边 task 节点重复创建。
        """
        async with self._conn() as conn:
            await conn.execute(
                self._sql("SELECT id FROM wf_process_instance WHERE id = ? FOR UPDATE"),
                (instance_id,)
            )
