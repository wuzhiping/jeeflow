# ./flows_demo 流程定义（Java 源参考版）

> **目录说明**：本目录是 jeeflow-java 仓 `test/resources/flows/` 的**精确镜像副本**，保留所有原始 SPI demo 用户 / role 引用（`leader` / `boss` / `manager` / `userA`/`userB`/`userC` 等）。**不参与运行时加载**，仅作为"对照参考"和"再生成流程定义"的输入。

## 用途

- **对照参考**：与 `./flows/` 对比，查看 spi.dev 适配做了什么用户/角色相关修改
- **再生成流程定义**：维护流程 = Java 源 → `_mirror()` → `./flows_demo/` → 基于本目录修改 → `./flows/`

## 运行时入口

引擎加载的是 `./flows/`（spi.dev 适配版），由 `flows_resolver.dir()` 返回。`./flows_demo/` 不会被引擎自动加载。

## 详见

- `../flows_resolver.py` — `_mirror()` 把 Java 源镜像到 `./flows_demo/`（不是 `./flows/`）
