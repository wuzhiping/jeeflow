...
演示组织结构 / 用户 / 角色 / 字典的 SPI（Service Provider Interface）实现集。
...
├── DEMO_USERS.json          # 8 个演示用户
├── DEMO_ROLES.json          # 4 个演示角色
├── DEMO_DICTS.json          # 2 组演示字典
├── DEMO_ROLE_TO_USERS.json  # 4 个角色-用户映射
│
...
### 2. impl SPI 入口（每个 `spi/demo/<func>.py`）
