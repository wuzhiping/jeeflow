...
- **v1.52** · 2026-11-17 · **users.md DM 执行 + 默认 SPI_FOLDER=dev 升级** · DM 通道 flowuser (W47 Day 5 月底 ping) → 0 新反馈诚实交代 (第 2 次, W40/W46/W47 三连) · 5 待办 (B7/B8 ID + FB-0011/0012 review + C-001 + FB-0013) 等您/bro · 6 处 docs/skills 升级标注"默认 SPI_FOLDER=dev" (推荐本地开发) · 代码层默认仍是 "demo" (spi/__init__.py:14, FREEZE.md 冻结) · 新增 retrospective 2026-11-17-flowuser-dm-no-feedback.md
- **v1.53** · 2026-11-17 · **FB-0012 patches 起草 (submitType=20 拓扑约束)** · 4 处 docs 修订 (flow.md §3.3 拓扑约束表 + AGENTS.md §5.8 提示框 + AGENTS.md §6 约束 #34 + actions.md §3 引用) + 1 处 known-issues.md §116 全文 · inbox/FB-0012.json → archive (received → 待 closed 复测) · 新增 retrospective 2026-11-17-fb-0012-patches.md · 等 bro apply + flowuser 复测 → FB-0012 status closed
- **v1.54** · 2026-11-17 · **🚨 重大 BUG 发现 · _cleanup_orphan_decision_tasks 函数签名回归 (FIX-T112)** · 切换 SPI_FOLDER=dev + 启动 main.py (8101) · 5 次独立 BDD 流程测试: 2 ✅ (01-simple + 04-fork-join 无 decision) + 1 ⚠️ (03-decision-expr 触达 TypeError) + 2 ❌ (15-decision-amount TypeError + 11-assignment-handler SPI 角色匹配空) · 根因: vendor/jeeflow/engine.py:710 函数定义 6 参数 vs 调用处 7 参数 (engine.py:680/690/705) · 影响所有多出边 decision 节点 (决策路由 80% 场景) · 建议修复: 函数签名 6 → 7 参数, 匹配调用处 · FB-0016 created · 等 bro apply FIX-T112-regression
- **v1.55** · 2026-11-17 · **FB-0016 二次验证 (demo 模式复现)** · 切换 SPI_FOLDER=demo 重启 main.py + 重跑 5 个独立 BDD 流程 · 结果与 dev 完全一致: 2 ✅ + 1 ⚠️ + 2 ❌ · **证实 BUG 与 SPI_FOLDER 无关** (engine.py 同代码) · bdd-1001-1060-p0-regression.sh demo 17/17 PASS (因使用 decisionHandler 而非原生 expr, 绕过 BUG) · FB-0016 更新含 demo 验证结果 · 修复优先级升级: P0 (影响所有多出边 decision 节点)
