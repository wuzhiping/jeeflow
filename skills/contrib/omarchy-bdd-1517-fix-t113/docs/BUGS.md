...
> 截至 2026-09-21，共完成 1203 个 BDD + 109 个 FIX (T1-T113)：
> - **109 个 FIX**：全部已修复 (T1-T113)
> - **0 个已知限制**：§2+§3+§4+§6+§7 + §113 全部完成
>
...
### FIX-T113 (2026-09-21): §116 decision expr 引用未声明变量 (W014 verify 拦截)

- **来源**: BDD-1517 5-run bug-hunt (2026-09-21, session `20260921_154407`, agent-driven memory-mode)
- **问题**: 流程 `bdd/bdd-1517-leave-reapply_20260921_220000.json` 的 decision1 出边 `e3` 表达式写 `days > 3`，但启动 `variables` 中只有 `f_days=5`。`SimpleExprEvaluator._eval_node(ast.Name("days"))` 执行 `vars["days"]` → KeyError → 抛 TypeError → 被 `except (TypeError, ValueError, KeyError, AttributeError)` 吞掉 → 返回 False → engine 兜底走默认边 `e4 → end1`。流程 state=20 DONE, 看起来"成功", 但 manager 节点永远未被访问, 审批记录只有 apply+leader 两条。
- **根因**: 流程设计者拼写错 (或漏掉 `f_` 前缀), 引擎 SimpleExprEvaluator 对 TypeError 静默处理 (FIX-T1 设计如此, 防止 string 评估崩溃整个流程); 但**没有 verify 规则拦截此类声明不一致**。
- **解决**:
  1. `vendor/jeeflow/verify.py` 新增 **W014** 规则: 解析 decision expr 提取标识符, 减去 `variables` keys + Python 关键字, 剩余 ≥2 字符的标识符 → 警告「expr 引用了未在 variables 中声明的字段」。
  2. 测试场景 `bdd/bdd-1517-leave-reapply-fix1_20260921_154132.json`: 把 `days > 3` 改为 `f_days > 3`, W014 不再触发, manager 节点正确创建。
  3. `docs/known-issues.md §116` 新增 BUG 详细说明 + 修复流程。
  4. `statics.json`: fix_total 108 → 109, verify_rules 32 → 33, warnings 12 → 13。
- **文件**: `vendor/jeeflow/verify.py` (W014) + `bdd/bdd-1517-leave-reapply-fix1_*.json` + `docs/known-issues.md §116` + `statics.json`
- **BDD**: `#1517-#1521` (5/5 PASS, 5-run bug-hunt)
- **复现实例**: `92135935274091` (BUG, 走默认边 end1) / `92136020572270` (FIX, 走 manager 边)
- **注**: 已存在的 `flows/03-decision-expr.json` 用 `amount > 1000` 但测试时同时传 `amount` + `f_amount` 两个 key, W014 不触发; 这是设计 agent 的"双 key 保险"模式, 现在被 W014 显式保护。
