...
## §116 FIX-T113 decision expr 引用未声明变量 (W014 verify 拦截) (2026-09-21)

> **来源**: BDD-1517 5-run bug-hunt, session `20260921_154407`, agent-driven memory-mode
...
```json
{"f_days": 5, "f_leaveType": "annual", "f_reason": "..."}
```

期望路径: apply → leader → decision1 → **manager** (因为 `f_days=5 > 3`) → end  
实际路径: apply → leader → decision1 → **end1** (默认边)

state=20 DONE, 看起来"成功", 但 manager 节点永远不创建, approvalRecord 只有 2 条 (apply/leader)。

### 根因

- 流程设计者拼写 expr 时漏掉 `f_` 前缀 (或拼错变量名)
- `SimpleExprEvaluator._eval_node(ast.Name("days"))` 执行 `vars["days"]` → KeyError → 抛 TypeError
- 被 `except (TypeError, ValueError, KeyError, AttributeError)` 吞掉 (FIX-T1 设计如此: 防止 string 评估崩溃整个流程)
- expr 返回 False → engine.py:670-693 兜底走第一条无 expr 边 (e4 → end1)
- W013 警告只检测「多分支 expr 缺失」, 不检测「expr 拼错」 → **verify 规则盲区**

### 实证 (BDD-1517 5-run bug-hunt)

| Run | flow | state | approvalRecord | 期望 |
|-----|------|-------|----------------|------|
| 1 (original) | `bdd-1517-leave-reapply_20260921_220000.json` | 20 DONE | apply+leader (2 条) | ❌ 应有 manager |
| 2-5 (original) | 同上 | 20 DONE | 同上 | ❌ 同一 bug |
| 1-5 (fix1) | `bdd-1517-leave-reapply-fix1_20260921_154132.json` | 20 DONE | apply+leader+manager (3 条) | ✅ |

**实例对比**:
- BUG 实例 `92135935274091`: `historyNodeNames=["apply","leader","decision1","end1"]`, 无 manager
- FIX 实例 `92136020572270`: `historyNodeNames=["apply","leader","manager","decision1","end","end1"]`, manager 出现

### 修复

**已实施 (FIX-T113 2026-09-21)**:

1. **`vendor/jeeflow/verify.py` 新增 W014 规则**:
   ```python
   W_DECISION_EXPR_UNKNOWN_VAR = "W014"
   # 检测: decision expr 引用的标识符不在 variables 中
   # 警告文案: "decision 节点[X] 的 expr 引用了未在 variables 中声明的字段: [...]. 
   #          引擎会因变量查找失败而走到默认边, 致该分支下游节点永远不创建。"
   ```

2. **`bdd/bdd-1517-leave-reapply-fix1_20260921_*.json`**:
   - 把 `expr: "days > 3"` 改为 `expr: "f_days > 3"` (与 variables key 对齐)
   - W014 不再触发, manager 节点正确创建

3. **`docs/known-issues.md §116`** (本文, 新增)

4. **`docs/BUGS.md FIX-T113`** (新增)

5. **`statics.json`**:
   - `fix_total`: 108 → 109
   - `verify_rules`: 32 → 33
   - `verify_breakdown.warnings`: 12 → 13
   - `bdd_total`: 1198 → 1203
   - `bdd_breakdown.section113_decision_expr_undeclared_var`: 新增 "5 (BDD #1517-#1521, 5-run bug-hunt PASS)"

### 反思 (避坑要点)

| 反模式 | 修正 |
|--------|------|
| decision expr 用 `days > 3` (裸名) | 用 `f_days > 3` (与 variables key 一致) |
| 用 `amount > 1000` 但变量只有 `f_amount` | 启动时同时传 `amount` + `f_amount`, 或改 expr 为 `f_amount > 1000` |
| 自测: instance.state=20 就认为通过 | 必须看 `approvalRecord` 节点顺序 + `highLight.historyNodeNames` 是否包含设计期望的所有节点 |

### 关联文档

- `vendor/jeeflow/verify.py` (W014 实现)
- `bdd/bdd-1517-leave-reapply-fix1_20260921_154132.json` (修复后流程)
- `bdd/BDD_5RUN_BUGHUNT_1517_FIX_T113_20260921_154407.md` (5-run bug-hunt 详细报告)
- `docs/BUGS.md FIX-T113`
- `docs/AGENTS.md §6 约束 #33` (待 bro apply, 新增)
