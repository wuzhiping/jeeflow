...
W_DECISION_EXPR_UNKNOWN_VAR = "W014"   # decision 出边 expr 引用的变量不在 variables 字典中（BUG-3 2026-09-21 BDD-1517 bug-hunt: `days > 3` 但实际变量是 `f_days`, 引擎兜底走默认边导致 manager 节点未被访问）
...
ALL_CODES = {
    ...
    W_DECISION_EXPR_UNKNOWN_VAR: "decision 出边 expr 引用了 variables 字典中没有的字段（引擎会静默走到默认边, BUG-3 §116 BDD-1517 2026-09-21）",
...
def verify_flow(flow: dict, variables: dict = None) -> Tuple[List[VerifyIssue], List[VerifyIssue], List[VerifyIssue]]:
    ...
    # W014 - decision 出边 expr 引用了未声明的变量 (BUG-3 2026-09-21 BDD-1517 bug-hunt)
    #   现象: 决策 expr 用 `days > 3`, 但 variables 只有 `f_days=5`,
    #         引擎 SimpleExprEvaluator 查 `vars["days"]` 返回 None → 抛 TypeError → 被 except 吞掉
    #         → expr 评估为 False → 兜底走默认边 (end1), **manager 节点永远不创建**
    #         流程 state=20 DONE, 看起来成功, 但下游 task 完全没走
    #   修复: 把 expr 改为 `f_days > 3` (与 variables key 对齐)
    #   检测:
    #     - 解析 expr 提取所有标识符 (跳过 Python 关键字/字面量/运算符)
    #     - 减去 variables 已知 key
    #     - 减去 Python 内置 (True/False/None/and/or/not)
    #     - 若还有剩余 → 标识符不在 variables → W014 warning
    ...
    for nid, n in node_by_id.items():
        ...
        for eid in out:
            ...
            ids = ids - var_keys
            for tok in ids:
                # 过滤单字母 (e.g. 数字比较里的 x, y) — 保守: 单字符不警告
                if len(tok) >= 2:
