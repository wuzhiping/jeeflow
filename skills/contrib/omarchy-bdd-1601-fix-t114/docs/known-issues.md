...
## §117 FIX-T114 (待修复) PARALLEL countersign REJECT 留下孤儿 DOING task (2026-09-21)

> **来源**: BDD-1601 5-run bug-hunt, session `20260921_155927`

### 现象

PARALLEL countersign 节点 (assignee=A,B,C; countersignType=PARALLEL; 未配置 ONE_VOTE_VETO) 成员之一用 `submitType=2 (REJECT)` 提交时:

| 期望 | 实际 |
|------|------|
| instance.state=45 | ✅ =45 |
| rejecter task.state=20 DONE | ✅ =20 |
| 其他 2 个会签成员 task.state=99 ABANDONED | ❌ **保持 10 DOING** |
| 其他节点 (若有) DOING 也清空 | ❌ 全部残留 |

### 根因

`vendor/jeeflow/engine.py:220 execute_and_jump_to_end` 仅调 `inst.reject()` + `update_instance`, 完全没动任务表:
...
对比 ONE_VOTE_VETO 路径 `engine.py:184-210`, 该路径有完整的 cleanup 逻辑:

```python
if ct:
    remaining = await self.repo.find_doing_tasks(inst.id, [cur_node.id])
    for t in remaining:
        t.abandon(now, abandoned_by=operator)  # ← FIX-T111 updateUser
    ...
    if cs_veto:
        ...
        for t in all_remaining:
            ...
            t.updateUser = operator  # ← 全部清
            await self.repo.update_task(t)
```

### 实证 (5-run, 5/5 复现)

| Run | rejecter | instance.state | rejecter task | 其他成员 task | 后续 execute |
|-----|----------|----------------|---------------|---------------|---------------|
| 1 | userA | 45 | state=20 | userB+C state=10 ❌ | "state=45 不可执行" |
| 2 | userB | 45 | state=20 | userA+C state=10 ❌ | 同上 |
| 3 | userC | 45 | state=20 | userA+B state=10 ❌ | 同上 |
| 4 | userA | 45 | state=20 | userB+C state=10 ❌ | 同上 |
| 5 | userB | 45 | state=20 | userA+C state=10 ❌ | 同上 |

### 影响

1. **Audit log 失真**: orphan task state=10 留在 audit log, 业务方审计时看到"未完成"的会签, 实际流程已 REJECT
2. **todoList 残留**: userB/C 仍能在 todoList 看到 task, 但 execute 报"实例 state=45 不可执行" (技术错误, 应提示"审批已结束")
3. **processTask/doneList 误算进度**: orphan DOING task 干扰统计
4. **FK 一致性**: 主流程 instance.state=45 但 task 表还有 DOING 行, 状态不一致

### 推荐修复 (FIX-T114)

**修改文件**: `vendor/jeeflow/engine.py:220-236`
...
async def execute_and_jump_to_end(self, task_id: int, operator: str, args: dict[str, Any] = None) -> ProcessInstance:
    ...
    # FIX-T114 §117 (2026-09-21)：拒绝路径必须清理 orphan DOING task
    ...
    now = datetime.now()
    if cur_node:
        # 当前会签节点的兄弟 DOING task 一律 ABANDON (aligns cs_veto path)
        remaining = await self.repo.find_doing_tasks(inst.id, [cur_node.id])
        for t in remaining:
            t.abandon(now, abandoned_by=operator)
            await self.repo.update_task(t)
            _sync_task_to_aggregate(inst, t)
        # 其他节点 (fork 子分支等) DOING 也清
        all_remaining = await self.repo.find_doing_tasks(inst.id)
        for t in all_remaining:
...
**约束**: 不能修改 `main.py` (BDD.md §4), 仅修改 `vendor/jeeflow/engine.py` (AGENTS.md §1 允许)

### 临时缓解 (不改引擎, 业务方)

| 方案 | 优点 | 缺点 |
|------|------|------|
| 全部会签配置 ONE_VOTE_VETO | 引擎已正确处理 | 失去"全员通过"语义 |
| 流程加 reject 边界 decision 节点 | 不改引擎 | 流程复杂, designer 容易漏 |
| 业务系统侧定时清理 orphan task | 即时生效 | 增加业务侧维护

### 关联文档

- `bdd/BDD_5RUN_BUGHUNT_1601_FIX_T114_20260921_155927.md` (待生成)
- `bdd/bdd-1601-expense-threshold-countersign_20260921_155448.json` (复现 flow)
- `bdd/run_5_times_1601_reject.py` (5-run 复现脚本)
- `vendor/jeeflow/engine.py:220-236` (待修改)
- `docs/BUGS.md §117` (BUG-4)
- `docs/AGENTS.md §6 约束 #34` (待 bro apply, 新增)
