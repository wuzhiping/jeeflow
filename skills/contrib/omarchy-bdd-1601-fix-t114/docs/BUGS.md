...
> 截至 2026-09-21，共完成 1203 个 BDD + 109 个 FIX (T1-T114)：
> - **108 个 FIX**：已修复 (T1-T111)
> - **1 个 FIX 起草中**：T114 (BUG-4 PARALLEL countersign REJECT 留下孤儿 DOING task)
...
### ❌ BUG-4 (FIX-T114 待起草): §117 PARALLEL countersign REJECT 留下孤儿 DOING task

- **来源**: BDD-1601 5-run bug-hunt (2026-09-21, session `20260921_155927`, agent-driven memory-mode)
- **优先级**: 高 (audit 失真 / orphan DOING task)
- **问题**: 流程 `bdd/bdd-1601-expense-threshold-countersign_20260921_155448.json` 配置 PARALLEL 会签节点 `countersign_finance` (assignee=userA,userB,userC, countersignType=PARALLEL) **且未配置** `countersignCompletionCondition: "ONE_VOTE_VETO"`。当任一会签成员 (userA/userB/userC) 用 `submitType=2 (REJECT)` 提交时, instance 立即被门面 `facade.py:731 execute_and_jump_to_end` 标记 state=45 (REJECT), **但其余 2 个会签成员的任务保持 state=10 (DOING) 不变**, 形成 orphan task。
- **根因**: `vendor/jeeflow/engine.py:220-236 execute_and_jump_to_end` 只调用 `inst.reject()`, 不调 `_cleanup_orphan_tasks` 清理当前节点或其他 DOING 任务。ONE_VOTE_VETO 路径 (`engine.py:184-210`) 已实现 cleanup, 但 submitType=2 (REJECT) 的 facade 路径未走该逻辑。
- **实证 (BDD-1601 5-run, 5/5 复现)**:
  | Run | rejecter | instance_state | userA | userB | userC |
  |-----|----------|----------------|-------|-------|-------|
  | 1 | userA | 45 (REJECT) | state=20 DONE | **state=10 orphan DOING** | **state=10 orphan DOING** |
  | 2 | userB | 45 (REJECT) | **state=10** | state=20 DONE | **state=10** |
  | 3 | userC | 45 (REJECT) | **state=10** | **state=10** | state=20 DONE |
  | 4 | userA | 45 (REJECT) | state=20 DONE | **state=10** | **state=10** |
  | 5 | userB | 45 (REJECT) | **state=10** | state=20 DONE | **state=10** |
  
  后续 userB/C 尝试 execute: `code=99999999 msg=[ValueError] 实例 state=45 不可执行任务` → 错误信息不友好 (用户预期"审批已结束", 实际报技术错误)
  
  Audit log 包含 5 条记录: apply/leader_review DONE + countersign_finance 一条 DONE (rejecter) + 2 条 state=10 DOING 永久存在
- **推荐修复 (FIX-T114, vendor/jeeflow/engine.py:220-236 修改建议, 待 bro apply)**:
  ```python
  async def execute_and_jump_to_end(self, task_id: int, operator: str, args: dict[str, Any] = None) -> ProcessInstance:
      ...
      # FIX-T114 (2026-09-21 §117)：REJECT 路径同样要清理 orphan DOING task
      # 复刻 cs_veto 路径 (engine.py:184-210) 的 abandon 逻辑, 但走 reject 而非 veto
      from .model import TaskState
      now = datetime.now()
      if cur_node:
          # 1) 当前节点 merged 触发的剩余 DOING task 一律 ABANDON, updateUser=rejecter
          ...
          # 2) 其他节点的 DOING task 也清 (与 cs_veto 路径一致)
      ...
      # ... (callActivity 主子回滚 + 事件触发保持不变)
  ```
- **临时缓解** (业务方可选, 不改引擎):
  1. 业务流程层面要求所有会签必走 `ONE_VOTE_VETO` 模式 (本场景 REJECT 行为已正确)
  2. 加一个独立的 reject 边界 decision 节点, 路由到 reject end (但会增加流程复杂度)
- **修复后**: 5 个 BDD 重测 PASS, 0 state=10 orphan, audit log 全部 DONE/ABANDONED
- **文件**: 待修复 `vendor/jeeflow/engine.py:220-236` + 测试 `bdd/run_5_times_1601_reject.py` (已留档)
- **BDD**: `#1601-#1605` (5/5 当前 FAIL, 修复后预期 PASS)
- **复现实例**: `92136993688802` (R1 userA reject 后 userB orphan), `92136993688803` (userC orphan), 5-run session `20260921_155927`
