...
```
2026-Q3 (4 周)              2026-Q4 (8 周)              2027-Q1~Q2 (16 周)        2026-Q4 (13 周)          2026-Q4 (7 周)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━  ━━━━━━━━━━━
[稳定化]                     [能力补齐]                  [生产化 + HA]              [§6 纵深优化]              [§7 双端深化 + 错误恢复]
引擎核心收敛                   限制消除 + 父子流程          监控 / 运维 / HA / 文档      关闭 §34/§36 + 导出           双端 BDD 自动化
████████████████ 完成 ✅    ████████████████ 完成 ✅    ████████████████ 完成 ✅    ████████████████ 完成 ✅    ░░░░░░ 推荐启动 ░░░░░
78 FIX / 1119 BDD          8 FIX (§3.1-§3.3)          14 FIX (§4.1-§4.4)         11 子任务 (§6 + FIX-T94~T104)   5 子任务 (§9.6)
verify 31 规则               TDD 17-suspend + 18-call   docs/openapi.json 70 paths   18/19 flows 双端一致       错误恢复 (SPI 暂停)
                                                       sla/check.sh 43/43 PASS    SPI 业务增强 (暂停)
                                                       ↑↑↑                          ↑↑↑
                                          ❌ 没有任何 UI/前端/可视化工作 (§5 已删除)              §7.1/§7.5/§7.4/§7.6 取消或暂停
                                          ❌ 没有任何 SDK/插件/第三方市场                           ❌ K8s/灰度/auth/UI
                                          (私用定位, 只服务本项目业务方)                          (私用定位, 业务方网关层处理)
```
...
**✅ 全部完成 (FIX-T94 ~ FIX-T103)**. 详见 [`TODO.md`](./TODO.md) (11 个子任务完整列表 + 验收标准 + 风险评估 + 依赖关系)。
...
### 9.6 §7 推荐路线图 ✅ **全部完成 (2026-09-20, 5/5, 实际 ~2 周)**

> **§7 背景**: §6 全部完成 (FIX-T94 ~ T104) + SLA v6 双端 + flows 验证后, 私用定位约束下, 推荐向**业务关键**方向发展。**§7 全部完成 (5/5, T105-T109, 实际 ~2 周 vs 预估 7 周)**。
> ❌ **§7.1 性能深挖已取消 (2026-09-20)**: MEM 1000 并发 P95=224ms / 287 inst/s, PG P95 <50ms, 当前规模性能充足, 进一步压测动力不足.
> **依据**: 扫描所有 `*.md` 文档 + SLA v6 数据 + 11-assignment-handler 唯一双端失败案例 + 角色信心指数 (运维 92% / 审计 95%)。
...
| 方向 | 子任务 | 工作量 | 实际 | 状态 | 价值 | 缺口来源 |
|------|--------|--------|------|------|------|----------|
| **§7.2 双端一致性深化** | §7.2.1 17 套 BDD 全量双端自动化 (FIX-T105) | 2 周 | 0.5 周 | ✅ DONE | SLA v6 进一步强化 | SLA.md v6 要求但当前仅在 check.sh 集成, 未作为 BDD |
| | §7.2.2 双端性能 diff 报告 (FIX-T106) | 1 周 | 0.3 周 | ✅ DONE | 性能基线对比 | 当前缺 PG vs MEM 性能对比 |

**P1 高 (4 周)**: 业务关键能力

| 方向 | 子任务 | 工作量 | 实际 | 状态 | 价值 | 缺口来源 |
|------|--------|--------|------|------|------|----------|
| **§7.3 错误恢复 + 事务** | §7.3.1 流程实例回滚 (FIX-T107) | 2 周 | 0.5 周 | ✅ DONE | 业务方运维痛点 | known-issues.md 未涉及, 业务实际高频 |
| | §7.3.2 CallActivity 主实例失败回滚 (FIX-T108) | 1 周 | 0.5 周 | ✅ DONE | 主子流程一致性 | §3.1.2 callActivity 已实现但主子回滚未做 |
| | §7.3.3 断点续跑 (FIX-T109) | 1 周 | 0.3 周 | ✅ DONE | 高可用强化 | §4.4 HA 已实现故障切换, 未实现任务恢复 |

**P2 中 (2 周)**: ⏸️ **无限期暂停 (2026-09-20)**

| 方向 | 子任务 | 状态 | 暂停理由 |
|------|--------|------|----------|
| ~~**§7.5 SPI 业务增强 (暂停)**~~ | ~~§7.5.1 补 11-assignment-handler 双端 BDD~~ | ⏸️ 暂停 | 11-assignment-handler 双端失败影响有限 (业务方生产用真实角色配置, SLA 不覆盖 SPI 角色匹配) |
| | ~~§7.5.2 SPI 角色配置工具 (避免手改 JSON)~~ | ⏸️ 暂停 | 业务方当前流程稳定, 工具化动力不足 |
| | ~~§7.5.3 业务 handler 模板库 (审批/通知/抄送)~~ | ⏸️ 暂停 | docs/known-issues.md §67 §68 已记录 handler 模式, 模板化非必需 |

**P3 低 (1 周+ 持续)**: 安全 + 可观测性深化

| 方向 | 子任务 | 工作量 | 价值 | 缺口来源 |
|------|--------|--------|------|----------|
| ~~**§7.4 安全 + 审计强化**~~ | ~~审计签名 / PG 行级权限 / 实例级审计~~ | — | §6.5.3 BUG 上报流程已就绪 | 私用定位, 业务方网关层处理 |
| ~~**§7.6 可观测性深化**~~ | ~~trace CLI / 流程级 metrics / alert 规则~~ | — | §4 + §6 已完整 | SLA 100% 已达成 |

**已取消 / 暂停**:
- ~~§7.1 性能深挖~~ — 当前规模性能充足 (MEM 1000 并发 P95=224ms)
- ~~§7.4 安全 + 审计强化~~ — 业务方网关层处理鉴权
- ~~§7.6 可观测性深化~~ — 当前 SLA 100%, 深化动力不足
- ⏸️ **§7.5 SPI 业务增强** (无限期暂停 2026-09-20) — 11-assignment-handler 双端失败业务影响有限, 业务方生产用真实角色配置

#### 9.6.2 §7 优先级速览

| 优先级 | 周数 | 状态 | 内容 | FIX |
|---|---|---|---|---|
| P0 立即 | 3 周 | ✅ DONE (0.8 周) | 双端一致性深化 (§7.2) | T105-T106 |
| P1 高 | 4 周 | ✅ DONE (1.3 周) | 错误恢复 + 事务 (§7.3) | T107-T109 |
| ~~P2 中~~ | ~~2 周~~ | ⏸️ **暂停** | ~~SPI 业务增强 (暂停) (§7.5)~~ | ~~T110-T112~~ |
| ~~P3 低~~ | — | ❌ 取消 | 安全 + 可观测性 (私用定位) | — |
| ~~**§7.1 性能深挖**~~ | ~~5 周~~ | ❌ **取消 (2026-09-20)** | 当前规模性能充足 | — |
